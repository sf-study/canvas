...
TICK_WIDTH=10,
TRACKING_DIAL_STROKING_STYLE='rgba(100,140,230,0.7)',
...
function drawGrid(color,stepx,stepy){
	...
	context.fillRect(0,0,context.canvas.width,canvas.height);
...
function drawDial(){
	...
	drawCentroid();//几何中心
	drawCentroidGuidewire(loc);//几何中心导丝
	drawRing();//圆环
	drawTickInnerCircle();//内部圆环的刻度
	drawTicks();
	drawAnnotations();//刻度仪表盘的数字
}
...
function drawTicks(){
	...
	ANGLE_MAX=2*Math.PI,ANGLE_DELTA=Math.PI/64,tickWidth;
	context.save();
